...
const Header = () => {
  ...
  return (
    <header className="w-full bg-[#c2b09a] text-stone-800 px-6 py-4 flex items-center justify-between font-sans relative z-[100]">
      {/* Logo & Title */}
      ...
      {/* Navigation */}
      <nav className={`hidden lg:flex items-center gap-5 xl:gap-7 font-medium tracking-wide ${language === 'ta' ? 'text-[11.5px] xl:text-[12.5px]' : 'text-[14px] xl:text-[15px]'
        }`}>
      ...
      >
        ...
        {/* Hamburger Menu (from Template, for aesthetics) */}
        <button className="flex flex-col gap-[5px] justify-center hover:opacity-75 transition-opacity lg:hidden">
          <span className="w-6 h-[1.5px] bg-stone-800"></span>
          <span className="w-6 h-[1.5px] bg-stone-800"></span>
        </button>
      </motion.div>
    </header>
